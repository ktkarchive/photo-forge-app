...
대량 인물 사진 선별을 위한 mac-first 데스크톱 도구입니다.
...
## macOS 설치
1. 릴리즈에서 `Photo-Forge-0.0.2-arm64.dmg` 다운로드
...
## Windows 설치 / 실행
### 설치형
- 릴리즈에서 `Photo-Forge-0.0.2-x64-installer.exe` 다운로드
- 실행 후 설치 경로 선택
- 바탕화면/시작 메뉴 바로가기 생성 가능

### 포터블
- 릴리즈에서 `Photo-Forge-0.0.2-x64-portable.exe` 다운로드
- 원하는 폴더에 두고 바로 실행
...
## 알려진 상태
- mac 빌드는 현재 가장 안정적입니다.
- Windows도 0.0.2 기준으로 설치형/포터블 재빌드 완료 상태입니다.
